#ifndef SFML1_GRAPH_CPP
#define SFML1_GRAPH_CPP

#include "Graph.h"

CircleClass::CircleClass():circle(sf::CircleShape()),id(-1){}
CircleClass::CircleClass(sf::CircleShape circle, int id):circle(circle),id(id){}
...
void Graph<T>::addEdge(const int& from, const int& to, int weight) {
    ...
    if(!fromVertex){
        fromVertex = addVertex(CircleClass(sf::CircleShape(),from));
    }
    Vertex<T>* toVertex = getVertex(to);
    if(!toVertex){
        toVertex = addVertex(CircleClass(sf::CircleShape(),to));
    }
...
Vertex<T> *Graph<T>::getVertex(const int &value) {
    for(int i=0; i < vertexList.size();i++ ){
        if(vertexList.get(i)->data.id == value) return vertexList.get(i);
    }
    return NULL;
}

template class Graph<CircleClass>;
